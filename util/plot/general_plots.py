...
if __name__ == "__main__":
    ...
    result_objects = {
        os.path.basename(filename).split(".")[0]: SuiteResult.load_hdf(
            filename
        )
        for filename in [
            os.path.join(args.in_dir, f"{ds}.h5") for ds in datasets
        ]
    }
    ...
    for metric_selection in ("all", "default"):
        k_a = {
            ds_name: compute_krippendorff_alpha(
                res, metric_selection=metric_selection
            )
            for ds_name, res in tqdm(result_objects.items())
        }
        k_a = pd.DataFrame(k_a)
        fig = plot_krippendorff_alpha(
            k_a,
            [
                "#036d08",
                "#9de78c",
        ...
        fig.savefig(
            os.path.join(args.out_dir, f"krip_bar_{metric_selection}.svg"),
            bbox_inches="tight",
        )
    sns.reset_orig()
        ...
        for ds_name, res_obj in tqdm(result_objects.items()):
            ...
            fig.savefig(
                os.path.join(
                    args.out_dir,
                    f"wilcoxon",
                    f"{ds_name}_{metric_selection}.svg",
                ),
                bbox_inches="tight",
            )
            plt.close(fig)
    ...
    for metric_selection in ("default", "all"):
        ...
        for ds_name, res_obj in tqdm(result_objects.items()):
            fig = inter_metric_correlations(
                res_obj, metric_selection=metric_selection
            )
    ...
    for ds_name in tqdm(["mnist", "cifar10", "imagenet"]):
        fig = pairwise_tests(
            result_objects[ds_name],
            "DeepSHAP",
            "DeepLIFT",
            title="P(DeepSHAP > DeepLIFT)",
        )
        # fig.savefig(os.path.join(args.out_dir, f"cles_{ds_name}.png"), bbox_inches="tight", dpi=250)
        fig.savefig(
            os.path.join(args.out_dir, f"cles_{ds_name}.svg"),
            bbox_inches="tight",
        )
        plt.close(fig)
    ...
    for metric_name in metric_names:
        ...
        fig.savefig(
            os.path.join(
                args.out_dir, "metric_corr_masking", f"{metric_name}.svg"
            ),
            bbox_inches="tight",
        )
        plt.close(fig)
    ...
    for ds_name in ["mnist", "cifar10", "imagenet"]:
        res_obj = SuiteResult.load_hdf(
            os.path.join(args.in_dir, f"{ds_name}.h5")
        )
