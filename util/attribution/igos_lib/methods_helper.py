...
def init(out_size):
    ...
    upscale.up = torch.nn.UpsamplingBilinear2d(size=(out_size, out_size))
...
def line_search(masks, total_grads, loss_func, alpha=8, beta=0.0001, decay=0.2,):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            select = indices.clone()
            indices[select] = losses > base_losses[select] + alphas[select] * t[select]
            # Same for this, but for if the alpha values are too low (\alpha_l)
            select = indices.clone()
            indices[select] *= (alphas[select] >= 0.00001)
...
def metric(image, baseline, mask, model, labels, step=5, size=28,):
    ...
    with torch.no_grad():
        ...
        up = torch.nn.UpsamplingBilinear2d(size=(out_size,out_size)).to(device)
        # True_mask is used to hold 1 or 0. Either show that pixel or blur it.
        true_mask = torch.ones((mask.shape[0], num_pixels)).to(device)
        del_scores = torch.zeros(mask.shape[0]).to(device)
        ins_scores = torch.zeros(mask.shape[0]).to(device)
