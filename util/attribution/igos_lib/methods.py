...
def IGOS(
    ...
    masks = torch.ones((images.shape[0], 1, size, size), dtype=torch.float32, device=device)
...
def iGOS_pp(
    ...
    masks_del = torch.ones((images.shape[0], 1, size, size), dtype=torch.float32, device=device)
    masks_del = Variable(masks_del, requires_grad=True)
    masks_ins = torch.ones((images.shape[0], 1, size, size), dtype=torch.float32, device=device)
...
def iGOS_p(
    ...
    for i in range(iterations):
        total_grads = torch.zeros(images.shape[0], 1, size, size, dtype=torch.float32).to(device)
        up_masks = upscale(masks)
