...
def extremal_perturbation(model,
    ...
    for t in range(max_iter):
        ...
        if variant == DELETE_VARIANT:
            ...
            x = torch.vstack([perturbations[i].apply(mask_[i]) for i in range(bs)])
        elif variant == DUAL_VARIANT:
            x = torch.cat((
        ...
        reward = reward.reshape(bs, -1).mean(dim=1)

        # Area regularization.
        mask_sorted = mask.reshape(bs, -1).sort(dim=1)[0]
