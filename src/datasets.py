...
class VOCDataset(Dataset):
    ...
    def __init__(self, root, train, transform):
        ...
        for cls in VOCDataset.classes:
            with open(
                    path.join(splits_dir, cls + '_' + self.split + '.txt')) as f:
                annotations[cls] = {line.split()[0]: line.split()[
                    1] == '1' for line in f.readlines()}
        self.annotations = pd.DataFrame.from_dict(annotations)
        assert (not (self.annotations.isna().any().any()
...
ALL_DATASETS = ["MNIST", "FashionMNIST", "CIFAR10", "CIFAR100", "SVHN",
                "ImageNet", "Places365", "Caltech256"]
SAMPLE_SHAPES = {
    "MNIST": (1, 28, 28),
...
def get_dataset(name, data_dir, train=False):
    ...
    if name == "MNIST":
        ...
        return datasets.MNIST(path.join(data_dir, "MNIST"),
        ...
        return datasets.FashionMNIST(path.join(
            data_dir, "FashionMNIST"), train=train, transform=transform,
            download=True)
    elif name == "CIFAR10":
        transform = transforms.Compose([
            transforms.ToTensor(),
        ...
        return datasets.CIFAR10(path.join(data_dir, "CIFAR10"),
                                train=train, transform=transform,
                                download=True)
    elif name == "CIFAR100":
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(mean=(0.5072, 0.4867, 0.441), std=[
                                 0.2673, 0.2565, 0.2762])
        ])
        return datasets.CIFAR100(path.join(
            data_dir, "CIFAR100"), train=train, transform=transform,
            download=True)
    elif name == "SVHN":
        ...
        return datasets.SVHN(path.join(data_dir, "SVHN"),
        ...
        dir = "train" if train else "val"
        return datasets.Places365(path.join(data_dir, 'Places365'), split=dir,
                                  transform=transform, small=True,
                                  download=False)
    elif name == "Caltech256":
        transform = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.ToTensor(),
            transforms.Normalize([0.552, 0.5332, 0.5047],
                                 [0.3154, 0.312, 0.3256])
        ])
        ...
        transform = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.ToTensor(),
            transforms.Normalize([0.4568, 0.4386, 0.4063], [
                                 0.2666, 0.2641, 0.2783])
        ])
        return VOCDataset(path.join(data_dir, 'VOC2012'),
                          train=train, transform=transform)
